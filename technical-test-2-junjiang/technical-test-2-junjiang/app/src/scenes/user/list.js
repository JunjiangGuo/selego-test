        ...
        <div className=" absolute top-0 bottom-0 left-0 right-0  bg-[#00000066] flex justify-center p-[1rem] z-50 " onClick={() => setOpen(false)}>
          <div
            ...
            {/* <Formik
              ...
              <Formik
                initialValues={{ username: "", email: "", password: "", status: "active", availability: "available" }}
                onSubmit={async (values, { setSubmitting }) => {
                  try {
                    const payload = {
                    ...
                    if (e.code === "DUPLICATE_FIELD") {
                      toast.error(`Duplicate ${e.field}: Please use a unique ${e.field}.`);
                    } else {
